    ...
    _timeout(1)
{
    ...
    if (_sock < 0) throw("FGFSSocket/socket");
...
int FGFSSocket::write(const char *msg, ...) {
    ...
    if (len < 0) throw("FGFSSocket::write");
    return len;
}
...
const char *FGFSSocket::read(void) {
    ...
    if (!select(FD_SETSIZE, &fd, 0, 0, &tv)) {
        ...
        else throw("FGFSSocket::read/select: timeout exceeded");
    }

    len = ::read(_sock, _buffer, BUFLEN - 1);
    if (len < 0) throw("FGFSSocket::read/read");
    if (len == 0) return 0;
